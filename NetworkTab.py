...
def network_kickoff(project_det, shared_state):
    ...
    def new_vpc():
        NewVPCRqstCol = [
            ...
            {'headerName': "Availability Zone", "field": "aZone"},
        ...
        with menu.form_card().classes("w-full"):
            with ui.grid(columns=2).classes("w-full"):
                global u_acc_Number, u_region, u_client, u_environment, u_AZone, u_HZone
                ...
                u_AZone       = menu.form_input('Availability Zone', "", "pl-6 w-full")
    ...
    def update_fields():
        if shared_state:
            u_acc_Number.set_value(shared_state.get('aws_account', ''))
            u_region.set_value(shared_state.get('region', ''))
            u_environment.set_value(shared_state.get('environment', ''))

    shared_state['update_network_tab'] = update_fields
